...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # TODO: I don't know how the fuck to start this

        # TODO: Find the closest ghost

        # Holds the closest ghost
        #closestGhostPosition = self.getClosestLivingGhost(livingGhostPositionDistribution)
        

        # TODO: Find the maze action to the closest ghost
    ...
    """
    # Gets the closest ghost
    def getClosestLivingGhost(self, distributions):
        # Holds the mininmum distance position as we go through the items
        currentGhostPositions = util.Counter()

        # Iterate over all the distributions
        for distanceDistribution in distributions:
            # Find the highest probability for the ghost
            maximumProbabilityIndex = distanceDistribution.argMax()

            # Holds the distance
    """


    # Just an inline placement that they gave us
    def getSuccessorPosition(self, position, action):
        return Actions.getSuccessor(position,action)
